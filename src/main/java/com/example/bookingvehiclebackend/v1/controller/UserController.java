...
public class UserController {
    ...
    public BaseApiResponse<?> payLateFee(@PathVariable String id) throws Exception {
        return BaseApiResponse.succeed(userService.payLateFee(id));
    }
    ...
    @GetMapping("/late/success")
    public void handlePaymentLateSuccess(@RequestParam("orderCode") long orderCode, HttpServletResponse response, HttpServletRequest request) throws IOException {
        // Tìm rental request dựa trên orderCode
        RentalRequest rr = rentalRequestRepository.findByOrderCode(orderCode)
                .orElseThrow(PvrsClientException.supplier(PvrsErrorHandler.RENTAL_REQUEST_NOT_FOUND));
        rr.setLateFeePaid(true);
    ...
    @GetMapping("/late/failed")
    public void handlePaymentLateFailed(@RequestParam("orderCode") long orderCode, HttpServletResponse response,HttpServletRequest request) throws IOException {
        // Tìm rental request dựa trên orderCode
        RentalRequest rr = rentalRequestRepository.findByOrderCode(orderCode)
                .orElseThrow(PvrsClientException.supplier(PvrsErrorHandler.RENTAL_REQUEST_NOT_FOUND));
        rr.setLateFeePaid(false);
        rentalRequestRepository.save(rr);
    ...
    public BaseApiResponse<?> getRentalListByUser(
        ...
        return BaseApiResponse.succeed(userService.rentalList(brands, categories, vehicleName, startDate, endDate, pageable, status,fuelType));
    }
